...
After the last step, the main entities of the RIC should be up and running in different dockers containers (this can be easily checked with the `docker ps` command). To understand what is going on in the RIC and to have a feedback once we will start ns-O-RAN, we can open two terminal for the RIC, one for logging the values on the E2Term and check the E2AP messages exchange, the other for the xapp.
{: .text-justify}
