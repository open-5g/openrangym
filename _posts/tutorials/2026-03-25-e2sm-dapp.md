...
The `--build-e3` flag enables the `E3_AGENT` compile flag, which activates IQ sample collection and the dynamic PRB blacklist.
...
Wait a little time before starting the xApp, this way is possible to see the logs of the dApp in the meantime.

The RIC must be running as first component so that the gNB E2 Agent can connects to it.
The gNB must be running before the dApp, so that this last can attach via E3, and the xApp needs to be executed last so that there is enough time to observe the dApp's logs.
...
Periodically, the list of identified PRBs will be written in the dApp log (`/tmp/dapp.log`).
