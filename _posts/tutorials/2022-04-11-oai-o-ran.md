...
This requires a [Colosseum](/experimental-platforms/colosseum) reservation with `oai-ric` and `oai-gnb-e2agent`. The procedure is similar to the gNB emulator case, with the following differences:
{: .text-justify}
...
Start the gNB in the SRN running with `oai-gnb-e2agent`:
