---
layout: post
title: "Deploying a dApp for Spectrum Sharing on a OpenAirInterface (OAI) gNB"
date: 2025-01-24
category: tutorials
...
dApps are software components that can execute as microservices, designed to be co-located with \glspl{cu} and \glspl{du}, where user-plane data is readily available.
Currently, the adoption of dApps is under investigation by the \oran \ngrg as a way to bring below 10\,ms \aiml routines to the \ran.
{: .text-justify}
...
We first start by cloning and setup the OAI repository with the following commands:
...
Then we can install and build OAI including the dApp extension, which is enabled by the flag `--build-e3`:
